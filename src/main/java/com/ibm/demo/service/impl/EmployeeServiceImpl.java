...
import com.ibm.demo.messaging.EmployeeEvent;
import com.ibm.demo.messaging.EmployeeMessageProducer;
...
public class EmployeeServiceImpl implements EmployeeService {
	...
	public EmployeeServiceImpl(EmployeeRepository employeeRepository, EmployeeMessageProducer messageProducer) {
	...
	public EmployeeResponse createEmployee(EmployeeRequest requestDTO) {
		...
		EmployeeResponse response = EmployeeMapper.toResponseDTO(saved);

		// Send JMS message to employee.created queue
		messageProducer.sendEmployeeCreatedEvent(new EmployeeEvent(saved.getId(), saved.getFirstName(),
				saved.getLastName(), saved.getEmail(), "CREATED"));
	...
	public EmployeeResponse updateEmployee(String id, EmployeeRequest requestDTO) {
		...
		EmployeeResponse response = EmployeeMapper.toResponseDTO(updated);

		// Send JMS message to employee.updated queue
		messageProducer.sendEmployeeUpdatedEvent(new EmployeeEvent(updated.getId(), updated.getFirstName(),
				updated.getLastName(), updated.getEmail(), "UPDATED"));
	...
	public void deleteEmployee(String id) {
		...
		// Send JMS message to employee.deleted queue
		// Note: we fetch the employee before deleting so we have the data for the event
		messageProducer.sendEmployeeDeletedEvent(new EmployeeEvent(employee.getId(), employee.getFirstName(),
				employee.getLastName(), employee.getEmail(), "DELETED"));
	}
}
